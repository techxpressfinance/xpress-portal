    ...
    <div className="mx-auto max-w-4xl">
        ...
        {editing ? (
          <div className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Select label="Priority" value={editPriority} onChange={(e) => setEditPriority(e.target.value)}>
          ...
          <>
            <div className="flex items-start justify-between gap-4 mb-4">
              <div className="min-w-0">
                ...
                {task.description && (
                  <p className="mt-1 text-[14px] text-muted-foreground">{task.description}</p>
                )}
              </div>
            ...
            <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-4">
              <div className="rounded-xl bg-secondary/50 p-3">
                <dt className="text-[12px] font-medium text-muted-foreground">Assigned To</dt>
                <dd className="mt-0.5 text-[14px] font-medium text-foreground">{task.assigned_to_name || 'Unassigned'}</dd>
              </div>
              <div className="rounded-xl bg-secondary/50 p-3">
                <dt className="text-[12px] font-medium text-muted-foreground">Due Date</dt>
                <dd className={`mt-0.5 text-[14px] font-medium ${isOverdue ? 'text-destructive' : 'text-foreground'}`}>
                  {task.due_date ? formatDate(task.due_date) : 'No due date'}
                </dd>
              </div>
              <div className="rounded-xl bg-secondary/50 p-3">
                <dt className="text-[12px] font-medium text-muted-foreground">Created By</dt>
                <dd className="mt-0.5 text-[14px] font-medium text-foreground">{task.created_by_name || 'Unknown'}</dd>
              </div>
              <div className="rounded-xl bg-secondary/50 p-3">
                <dt className="text-[12px] font-medium text-muted-foreground">Created</dt>
                <dd className="mt-0.5 text-[14px] font-medium text-foreground">{formatDate(task.created_at)}</dd>
              </div>
            </dl>
      ...
      <GlassCard>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[16px] font-semibold text-foreground">Checklist</h2>
          {totalItems > 0 && (
        ...
        {totalItems === 0 && !newItemTitle && (
          <p className="text-[13px] text-muted-foreground mb-4">No checklist items yet. Add one below.</p>
        )}
        ...
        <div className="space-y-1">
          {task.checklist_items.map((item) => (
            <div
              key={item.id}
              className="group flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-secondary/50 transition-colors"
            >
              <button
