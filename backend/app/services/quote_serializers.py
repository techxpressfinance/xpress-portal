...
def serialize_quote_sheet(sheet: QuoteSheet) -> dict:
    return {
        ...
        # Lender pricing only — null throughout on a client quote.
        "lender_id": sheet.lender_id,
        "lender_name": sheet.lender.name if sheet.lender else None,
        "asset_price": float(sheet.asset_price) if sheet.asset_price is not None else None,
        "deposit_amount": float(sheet.deposit_amount) if sheet.deposit_amount is not None else None,
        "trade_in_amount": float(sheet.trade_in_amount) if sheet.trade_in_amount is not None else None,
        "payout_amount": float(sheet.payout_amount) if sheet.payout_amount is not None else None,
        "amount_borrowed": float(sheet.amount_borrowed) if sheet.amount_borrowed is not None else None,
        "shortfall_accepted": sheet.shortfall_accepted,
        "shortfall_bypassed": sheet.shortfall_bypassed,
        "shortfall_notes": sheet.shortfall_notes,
